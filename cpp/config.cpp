...
int main(int argc, char* argv[]) {
    ...
    out << "transport.spread.host: " << spread.getOptions().get<string>("host") << endl;
    out << "transport.spread.port: " << spread.getOptions().get<string>("port") << endl;
    out << "transport.spread.enabled: " << spread.isEnabled() << endl;

    ParticipantConfig::Transport::ConverterNames names = spread.getConverters();

    for (ParticipantConfig::Transport::ConverterNames::const_iterator it = names.begin(); it != names.end(); ++it) {
	if (it->first == "string") {
	    out << "transport.spread.converter.cpp.string: " << it->second << endl;
	}
    }
